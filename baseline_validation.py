...
def test_forward_pass(model, config):
    ...
    try:
        ...
        # 验证输出维度
        expected_shape = (batch_size, n_frames, num_joints,
                          config.get('dim_out', 3))
...
def simulate_training_step(model, config):
    ...
    try:
        ...
        x = torch.randn(batch_size, n_frames, num_joints,
                        config.get('dim_in', 3))
        y = torch.randn(batch_size, n_frames, num_joints,
                        config.get('dim_out', 3))
